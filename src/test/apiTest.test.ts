...
describe('Session and device for User /SecurityDevices', ()=> {
    ...
    it('add many devices for one user',async ()=>{
        ...
        const refreshTokenCookies2=cookies2.filter(function(val:string){return val.split('=')[0]=='refreshToken'})
        const refreshToken2= refreshTokenCookies2.map(function (val:string){return val.split('=')[1]}).map(function(val:string){return val.split(';')[0]})[0]
        expect(refreshToken2).toEqual(expect.any(String))
        //

        //refresh3
        const cookies3 = AddDevice3Response.headers['set-cookie']
        const refreshTokenCookies3=cookies3.filter(function(val:string){return val.split('=')[0]=='refreshToken'})
        const refreshToken3= refreshTokenCookies3.map(function (val:string){return val.split('=')[1]}).map(function(val:string){return val.split(';')[0]})[0]
        expect(refreshToken).toEqual(expect.any(String))
        //

        //lastActiveDate and deviceId2
        const lastActiveDate2=await jwtService.getLastActiveDateFromRefreshToken(refreshToken2)
        const verifyToken2=await jwtService.verifyToken(refreshToken2)
        const deviceId2=verifyToken2.deviceId
        //

        //lastActiveDate and deviceId3
        const lastActiveDate3=await jwtService.getLastActiveDateFromRefreshToken(refreshToken3)
        const verifyToken3=await jwtService.verifyToken(refreshToken3)
        const deviceId3=verifyToken3.deviceId
        //
